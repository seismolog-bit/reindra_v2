...
@section('content')
    ...
    <h2 class="text-bold text-body-emphasis mb-5">Theme tag</h2>
                                ...
                                <td class="title align-middle white-space-nowrap"><a
                                        ...
                                        <div class="avatar avatar-m">
                                            <div class="avatar-name rounded-circle">
                                                <span>{{ substr($themeTag->title, 0, 1) }}</span>
                                            </div>
                                        </div>
                                        <h6 class="mb-0 ms-3 fw-semibold">{{ $themeTag->title }}</h6>
                                    </a></td>
                                <td class=" align-middle text-end white-space-nowrap text-body-tertiary">
                                    <div class="col-12 col-md-auto d-flex justify-content-end">
                                        <button type="button" class="btn btn-phoenix-primary btn-icon me-2"
                                            onclick="openEditModal({{ $themeTag->id }})">
                                            <span data-feather="edit"></span>
                                        </button>
                                        <form method="POST" action="{{ route('admin.theme-tag.destroy', $themeTag) }}"
                                            class="d-inline"
...
@section('modal')
    @include('components.modals.theme-tag')
@endsection
...
@section('script')
    ...
    <script>
        function openEditModal(pricingId) {
            ...
            $.get('/admin/theme-tag/' + pricingId + '/edit/', function(data) {
